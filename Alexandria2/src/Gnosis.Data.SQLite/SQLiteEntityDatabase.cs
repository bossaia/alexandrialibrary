...
{
    public abstract class SQLiteEntityDatabase<T>
        : IEntityStore<T>
        where T : IEntity
    {
        protected internal SQLiteEntityDatabase(string databaseName, string tableName)
        {
        ...
        protected virtual void LoadLink(IDataRecord record, Action<uint, ILink, uint> linkLoaded)
        {
        ...
        protected virtual void LoadTag(IDataRecord record, Action<uint, ITag, uint> tagLoaded)
        {
            var id = (uint)record.GetInt64(0);
            var parent = (uint)record.GetInt64(1);
            var name = record.GetString(2);
        ...
        protected virtual void LoadLinks(IDbConnection connection, Action<uint, ILink, uint> linkLoaded)
        {
            if (linkLoaded == null)
                return;
        ...
        protected virtual void LoadTags(IDbConnection connection, Action<uint, ITag, uint> tagLoaded)
        {
            if (tagLoaded == null)
                return;
        ...
        protected virtual IStep GetUpdateLinkStep(uint id, ILink link, uint entityId)
        {
        ...
        protected virtual IStep GetUpdateTagStep(uint id, ITag tag, uint entityId)
        {
            var step = new Step(insertTagCommandText);
            step.AddItem(parentParameterName, entityId);
            step.AddItem("@Name", tag.Name);
        ...
        public void Initialize(Action<uint, T> entityLoaded, Action<uint, ILink, uint> linkLoaded, Action<uint, ITag, uint> tagLoaded)
        {
        ...
        public uint CreateLink(IBatch<T> batch, ILink link, uint entityId)
        {
        ...
        public uint CreateTag(IBatch<T> batch, ITag tag, uint entityId)
        {
        ...
        public void SaveLink(IBatch<T> batch, uint id, ILink link, uint entityId)
        {
            batch.Execute(GetUpdateLinkStep(id, link, entityId));
        }

        public void SaveTag(IBatch<T> batch, uint id, ITag tag, uint entityId)
        {
            batch.Execute(GetUpdateTagStep(id, tag, entityId));
        }

        public IBatch<T> CreateBatch(IEntityCache<T> cache)
        {
