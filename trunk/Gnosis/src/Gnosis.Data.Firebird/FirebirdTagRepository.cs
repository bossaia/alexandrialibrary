        ...
        : FirebirdRepositoryBase
    {
        ...
        public void Initialize()
        {
            try
            {
                ...
                tableSql.AppendLine("execute statement 'create table Tag (Id BIGINT NOT NULL, Target VARCHAR(4000) NOT NULL, Type BIGINT NOT NULL, Name VARCHAR(4000) NOT NULL, PRIMARY KEY (Id));';");
            ...
            using (var connection = GetConnection())
            {
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        var type = TagType.Parse(reader.GetInt64("Type"));
                        var name = reader.GetString("Name");
                        var id = reader.GetInt64("Id");

                        tags.Add(new Tag(target, type, name, id));
                    }
                }
            }
        ...
        public void Save(IEnumerable<ITag> tags)
        {
            try
            {
                ...
                foreach (var tag in tags)
                {
                    ...
                    sql.AppendLine("insert into Tag (Target, Type, Name) values (@Target, @Type, @Name)");
