...
namespace Gnosis.Application.Vendor
{
    public class GnosisDoc
        : IDoc
    {
        public GnosisDoc(string title, uint number, Uri creator, string creatorName, Uri album, string albumTitle, Uri target, IMediaType targetType, Uri thumbnail)
            : this(title, number, creator, creatorName, album, albumTitle, target, targetType, thumbnail, Guid.NewGuid().ToUrn())
        {
        }

        public GnosisDoc(string title, uint number, Uri creator, string creatorName, Uri album, string albumTitle, Uri target, IMediaType targetType, Uri thumbnail, Uri location)
        {
            if (title == null)
        ...
        {
            get { return MediaType.ApplicationGnosisDoc; }
        }
