//This code is mostly borrowed from elsewhere.
//I'm sure the original author won't mind it's use here, otherwise it can be rewritten from scratch.
using System;
		...
		private int HookProc(int hdlg, int msg, int wParam, int lParam)
		{
			switch (msg)
			{
				case WM_INITDIALOG:
					...
					SetWindowText(labelHandle, "&Format:");
					...
					int comboHandle = CreateWindowEx(0, "ComboBox", "coolcombobox", WS_VISIBLE | WS_CHILD | CBS_HASSTRINGS | CBS_DROPDOWNLIST | WS_TABSTOP | WS_DISABLED, point.X, point.Y + 8, rightPoint.X - point.X - 75, 100, parent, 0, 0, 0);
					...
					int buttonhandle = CreateWindowEx(0, "Button", "&Modify Presets", WS_DISABLED | WS_VISIBLE | WS_CHILD | WS_TABSTOP, point.X + 175, point.Y + 8, 75, 23, parent, 777, 0, 0);
					SendMessage(buttonhandle, WM_SETFONT, fontHandle, 0);
					...
					break;
				case 0x111:
					if ((wParam & 0xFFFF) == 777)
					{
						MessageBox.Show("Alice\'s House");
					}
					break;
				case WM_DESTROY:
