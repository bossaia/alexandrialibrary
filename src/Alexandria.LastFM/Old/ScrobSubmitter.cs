...
{
	public class ScrobSubmitter : IDisposable
	{
		#region Constructors
		public ScrobSubmitter()
		{
		...
		const int defaultPort = 33367;
		const int launchWait = 60000; //in milliseconds
		const string pluginId = "ALX";
		#endregion
		
		#region Private Fields
		private bool disposed;
		private BlockingClient client = new BlockingClient();
		private object submitterLock = new object();
		#endregion
		
		#region Private Methods
		private void InitializeCriticalSection()
		{
		}
		
		private void DeleteCriticalSection()
		{
		}
		#endregion
		...
		public int Start(string artist, string track, string album, string musicBrainzId, int length, string fileName, Encoding encoding)
		{
			System.Text.StringBuilder command = new StringBuilder("START ");
			command.Append("c=" + pluginId + "&");
			command.Append("a=" + Escape(artist) + "&");
			command.Append("t=" + Escape(track) + "&");
			command.Append("b=" + Escape(album) + "&");
			command.Append("m=" + Escape(musicBrainzId) + "&");
			command.Append("l=" + length.ToString() + "&");
			command.Append("p=" + Escape(fileName));
		
			return SendToAudioScrobbler(command.ToString());
			
			#region Legacy C++ Code
			/*
		...
		public int SendToAudioScrobbler(string command)
		{
			return -1;
		}
		
		public void SendToAudioScrobblerThread()
		{
		}
		
		public void ConnectToAudioScrobbler()
		{
		}
		
		public string ConvertToUtf8(string text, Encoding encoding)
		{
			return text;
		}
