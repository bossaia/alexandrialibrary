        ...
        private void LoadPodcastStarted(object sender, DoWorkEventArgs args)
        {
            try
            {
                ...
                if (lastBuildDateNode != null)
                {
                    ...
                    Gnosis.Core.Time.Rfc822DateTime.TryParse(lastBuildDateNode.InnerText, out lastBuildDate);
                ...
                if (itemNodes != null && itemNodes.Count > 0)
                {
                    foreach (XmlNode itemNode in itemNodes)
                    {
                        ...
                        if (!string.IsNullOrEmpty(path))
                        {
                            ...
                            if (dateNode != null)
                                Gnosis.Core.Time.Rfc822DateTime.TryParse(dateNode.InnerText, out date);
